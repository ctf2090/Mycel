...
MycelLayer 是 Mycel 的對外名稱，用來表達它是一個以 Rust 實作為主、面向可驗證修訂歷史、依規則導出的預設閱讀版本，以及去中心化複製的協定。

在這個 repo 與整套規格文件中，正式的技術名稱仍然維持為 `Mycel`。

它面向的是需要以下能力的以文本為先的系統：

- 可重播驗證的修訂歷史
- 可驗證的簽章治理訊號
- 多個有效分支並存，而不要求全域強制共識
- 由固定規則組（profile）導出的預設閱讀版本，而不是任意本地偏好
...
Mycel 走的是另一條路。它把修訂歷史、預設閱讀版本，以及去中心化複製視為彼此分離但可互通的層次。

因此，它特別適合長期文本、評註系統、受治理的參考文本集合，以及其他以文字為核心的分散式工作流。

它補上了中央化協作平台與全域共識區塊鏈之間的空缺：讓歷史可驗證、預設閱讀版本可由規則導出、資料可去中心化複製，同時不必強迫整個網路收斂成唯一的真版本。

## 它的差異點

- 可驗證的修訂歷史：修訂內容預期要能被重播驗證與檢查，而不是只靠信任。
- 依規則導出的預設閱讀版本：預設採用的版本來自固定規則組與已驗證的 View 物件。
- 容許分叉：多個 head 可以並存，而不假設整個網路必須只有一個全域真相。
- 中立的協定核心：領域語義應放在 profiles 與應用層，不應寫死進協定核心。

換句話說，所謂「採用版本」不是全網共識，而是在某個固定規則組下，從已驗證物件推導出的版本。
...
目前的 Rust CLI 是內部驗證與模擬器工具鏈，還不是正式可上線的 Mycel 客戶端或節點。
...
- `info`：repo 內部工作區與骨架目錄路徑
- `validate`：對已提交測試樣本的穩定驗證輸出
- `sim run`：模擬器測試框架的執行摘要，以及產生出的報告路徑
...
如果你想看更結構化的任務入口，請直接瀏覽帶有 `ai-ready` 與 `well-scoped` 標籤的 issues。

如果你想理解目前的 Rust 實作：

- 工作區地圖：[RUST-WORKSPACE.md](./RUST-WORKSPACE.md)
- 模擬器骨架：[sim/README.md](./sim/README.md)
...
- [PROTOCOL.zh-TW.md](./PROTOCOL.zh-TW.md)：協定核心規格
- [WIRE-PROTOCOL.zh-TW.md](./WIRE-PROTOCOL.zh-TW.md)：傳輸訊息格式與 sync flow 草案
- [ROADMAP.zh-TW.md](./ROADMAP.zh-TW.md)：從第一個客戶端到後續擴展的分階段建置順序
- [IMPLEMENTATION-CHECKLIST.zh-TW.md](./IMPLEMENTATION-CHECKLIST.zh-TW.md)：窄版可互通客戶端的實作檢查清單
- [PROFILE.fund-auto-disbursement-v0.1.zh-TW.md](./PROFILE.fund-auto-disbursement-v0.1.zh-TW.md)：窄版應用層 custody profile 草案
