...
| accepted reading | 採信閱讀結果 | 指 reader 在固定 profile 下預設採用的閱讀版本。 |
...
| reader client | reader client / 閱讀 client | 顯示 document family 並導出 accepted reading 的客戶端。 |
...
| deterministic | 可決定 / 決定性 | 輸入相同時結果可重現。 |
...
- Mycel 是一個用於可驗證文本歷史、受治理閱讀狀態與去中心化複製的協議棧。
- 所謂「採信版本」不是全網共識，而是依固定 profile 規則，從已驗證物件推導出的結果。
- `Document` 在 Mycel 中是長期可重播的物件歷史，不必然是傳統文字文件。
- app-layer 語意應放在 profiles 與 applications，不應寫死進 protocol core。
