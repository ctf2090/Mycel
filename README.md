...
- Current topology-validation scope: `validate sim/topologies --json` validates the topology set as a repo-scoped input and surfaces related test-case and report counts alongside the topology counts
- Current peer-validation loading boundary: peer-scoped validation now loads related topologies, test cases, and reports from the same repo when they reference the selected peer node IDs, so `validate sim/peers --json` no longer treats peer fixtures as isolated by default
...
- Current info/help output boundary: `info` prints stable workspace/scaffold path sections for humans, while `help` and no-arg invocation print the top-level usage contract and unknown commands fail after printing that usage
