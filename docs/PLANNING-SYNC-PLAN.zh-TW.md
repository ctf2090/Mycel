...
- `sync doc`：只同步 Markdown。包含 `ROADMAP.*`、`IMPLEMENTATION-CHECKLIST.*`、`docs/PROGRESS.md`，以及相關 README 文案。
- `sync web`：只同步 GitHub Pages。包含 `pages/progress.html` 與各語系 landing pages 中不屬於 issue 入口的 HTML 摘要文案。
...
- `docs/PROGRESS.md` 與 `pages/progress.html` 是衍生摘要，不得自行發明專案狀態。
...
用 `docs/PROGRESS.md` 與 `pages/progress.html` 回答：
...
4. 若公開摘要用語改變，再更新 `pages/progress.html`
...
- `sync doc` 在 10 commits 時到門檻
- `sync issue` 在 10 commits 時到門檻
- `sync web` 在 20 commits 時到門檻

若它回報 `due`，則在下一個 planning-sync batch 更新被點名的表面：
...
- 對齊的 GitHub Issues
- 屬於 Markdown 的 planning 摘要層，例如 `docs/PROGRESS.md`，在 `sync doc` 到門檻時更新
- 對齊的 GitHub Issues，在 `sync issue` 到門檻時更新
- GitHub Pages 上的 HTML 摘要層，例如 `pages/progress.html` 與各語系 landing pages 的非 issue 文案，在 `sync web` 到門檻時更新
...
3. `doc` 必須執行 planning-refresh cadence checker（規劃同步節奏檢查工具）
4. 若 `sync doc` 到門檻，refresh `ROADMAP.md`、`IMPLEMENTATION-CHECKLIST.*`、`docs/PROGRESS.md` 與相關 README 文案
5. 若 `sync issue` 到門檻，對齊 GitHub Issues
6. 若 `sync web` 到門檻，再更新 `pages/progress.html` 與各語系 landing pages 的非 issue HTML 摘要
...
2. 除非目前 mailbox 明確指向 archive 裡仍未解決的條目，否則不要回頭掃 archived mailboxes
3. `doc` 必須執行 planning-refresh cadence checker（規劃同步節奏檢查工具）
4. 更新 `pages/progress.html` 與各語系 landing pages 的非 issue HTML 摘要
...
6. landing page 的 contributor-entry 連結指向過期、已關閉、或已不具代表性的 issue
...
- `pages/progress.html` 與 `docs/PROGRESS.md` 一致
- `pages/index.html` 與各語系 landing pages 中的 contributor-entry 連結仍指向具代表性的 open starter issues
...
4. 把 `docs/PROGRESS.md` 與 `pages/progress.html` 視為公開摘要層
5. 把 README 的 contributor 指引視為 Markdown 文案；README 只需要指向 GitHub issue 清單，不在那裡精選 starter issues
6. 把 `pages/index.html` 與各語系 landing pages 中的 contributor issue 連結視為公開的 curated issue 入口，並在 planning sync 時一起刷新
