...
- When a work cycle produces any source changes, include a files-changed summary in the final reply before the next-stage options. Run `scripts/render_files_changed_table.py HEAD` (or the relevant commit/range) and paste its raw stdout output verbatim into the reply; do not reformat, translate, or replace the generated table with a hand-written version. <!-- item-id: workflow.files-changed-summary -->
