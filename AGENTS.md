...
- In the final reply for every completed work cycle, include a rough estimated token-usage figure or range for that cycle, and clearly label it as an estimate unless an observed UI or API value is being reported instead. Use the token report format `估計 token 使用量：xxK / xxK。` <!-- item-id: workflow.report-estimated-token-usage -->
