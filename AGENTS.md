...
- If `.agent-local/dev-setup-status.md` says `Status: ready`, treat that as static workspace readiness only; before running tests, scripts, `cargo run`, or `cargo test`, do a lightweight runtime preflight with `scripts/check-runtime-preflight.sh` plus any shell utilities used in the exact command (for example `--require grep` or `--require tail`). If a required command is missing or a command exits `126`/`127`, report it as an environment block first and do not treat the result as product validation or checklist closure. <!-- item-id: workflow.runtime-preflight-before-verification -->
...
- When this agent's current work cycle produces any source changes, include a files-changed summary in the final reply before the next-stage options. Pre-existing unrelated worktree changes do not trigger this requirement by themselves. Run `scripts/render_files_changed_table.py HEAD` (or the relevant commit/range) and paste its raw stdout output verbatim into the reply as a bare Markdown table; do not wrap it in a fenced code block, and do not reformat, translate, or replace the generated table with a hand-written version. <!-- item-id: workflow.files-changed-summary -->
...
- `scripts/agent_work_cycle.py end` now hard-fails clean closeout for cycles with source changes unless the current `HEAD` is already reachable from `origin/main`; commit and push the agent's own source-change commit before treating the cycle as complete.
