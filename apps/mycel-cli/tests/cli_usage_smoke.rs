...
#[test]
fn view_current_help_prints_structured_clap_help() {
    let assert = mycel_command(&["view", "current", "--help"])
        .assert()
        .success();
    let stdout = assert_stdout_text(&assert);

    assert_eq!(assert_stderr_text(&assert), "");
    assert_view_current_help(&stdout);
}
...
#[test]
fn view_document_help_prints_structured_clap_help() {
    let assert = mycel_command(&["view", "document", "--help"])
        .assert()
        .success();
    let stdout = assert_stdout_text(&assert);

    assert_eq!(assert_stderr_text(&assert), "");
    assert_view_document_help(&stdout);
}

#[test]
fn view_maintainer_help_prints_structured_clap_help() {
    let assert = mycel_command(&["view", "maintainer", "--help"])
        .assert()
        .success();
    let stdout = assert_stdout_text(&assert);

    assert_eq!(assert_stderr_text(&assert), "");
    assert_view_maintainer_help(&stdout);
}

#[test]
fn view_list_help_prints_structured_clap_help() {
    let assert = mycel_command(&["view", "list", "--help"])
        .assert()
        .success();
    let stdout = assert_stdout_text(&assert);

    assert_eq!(assert_stderr_text(&assert), "");
    assert_view_list_help(&stdout);
}
...
fn store_index_help_prints_structured_clap_help() {
    ...
    assert!(stdout.contains("--maintainer <MAINTAINER>"));
