...
Status: draft, refreshed after the implementation checklist was split into a closed `M1` gate plus a live post-`M1` follow-up checklist so the summary now reflects `M2` / `M3` / `M4` as the active planning lane; `M2` now records the newer nested/anchor competing-placement classification growth plus a clearer metadata-merge boundary where non-primary-add and keep-primary-over-add branches are landed while metadata removal remains a v0.1 patch-model boundary, and `M4` records localhost transport proof, all three currently tracked production replication sub-proofs, and a first permanent messages-after-BYE session proof as landed
...
| `M2` | Substantially underway | replay, `state_hash`, store rebuild, ancestry-aware render/store verification, narrow write path, and conservative merge authoring with broader structural coverage plus manual-curation smoke for nested parent-choice, nested sibling-choice, and composed-branch placement conflicts, alongside richer direct and anchor-based competing placement reasons and a clearer metadata split between adopted non-primary adds, kept-primary competing adds, and manual-curation-only removals | stronger replay/store fixtures, broader core reuse, richer content competing-variant classification, and any later decision about whether metadata deletion should remain outside v0.1 patch ops |
...
| 10. Merge Generation | Partial | `M2` | replay verification and a conservative local merge-authoring profile exist, including structural move/reorder, new-parent reparenting, simple composed parent-chain coverage, a broader nested structural matrix, manual-curation smoke for nested parent-choice, nested sibling-choice, and composed-branch placement conflicts, richer direct/anchor-based competing placement reasons, landed metadata competing-variant handling for adopting or keeping primary over non-primary metadata additions, and an explicit manual-curation boundary for metadata removal because v0.1 patch ops do not yet express deletion; broader content classification and any future metadata-deletion decision still remain |
| 11. CLI or API Surface | Partial | `M2` / `M3` / `M4` | verification, authoring, conservative merge authoring, editor-admission-aware reader inspection/render, governance inspect/list/publish, persisted governance index query surfaces, transcript-backed `sync pull`, and internal `sync peer-store` all exist, including optional snapshot/view flows, localhost multi-process proof, re-sync idempotency, depth-N catchup, partial-doc selective sync, and messages-after-BYE rejection; the remaining `M4` gap is broader session/error-path interop proof |
| 12. Interop Test Minimum | Partial | `M1` / `M2` / `M4` | fixture isolation, reproducibility, stricter parser/replay smoke coverage, direct wire-envelope/signature/session tests, peer-store first-time / incremental sync proofs, optional-message coverage, localhost multi-process proof, re-sync idempotency coverage, depth-N incremental catchup coverage, partial-doc selective sync coverage, and messages-after-BYE coverage exist, but broader session/error-path cases are still open |
