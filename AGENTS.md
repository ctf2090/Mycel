...
- When a work cycle produces any source changes, include a files-changed summary in the final reply before the next-stage options. Use `scripts/render_files_changed_table.py HEAD` (or the relevant commit/range) to generate the Markdown table with columns: File, +/-, and a one-line note per file. <!-- item-id: workflow.files-changed-summary -->
