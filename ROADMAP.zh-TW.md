...
6. 提供獨立於 reader-facing `head` commands 的 `view inspect` / `view publish` governance workflows
7. simulator 與 validation workflows，涵蓋 peer、topology、test 與 report 範圍

主要剩餘缺口：

1. 更廣泛的 governance-state persistence
2. 超出最小具名 `view inspect` / `view publish` workflow 的專用 governance inspection / publication surfaces
