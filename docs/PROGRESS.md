...
Status: draft, refreshed after the recent canonical-helper convergence, peer-store sync-driver, CLI peer-sync, and simulator integration batch
...
4. keep `M4` narrow while peer-store first-time and incremental sync proofs grow toward optional-message and broader interop closure
...
| `M4` | Early partial | wire envelope validation, `OBJECT` body verification, session reachability, store-backed bootstrap, and peer-store-driven first-time / incremental sync proofs | capability-gated optional flows, broader peer interop proof, and production replication behavior remain |
...
| 7. Wire Protocol | Partial | `M4` | canonical wire-envelope parsing, field validation, RFC 3339 checks, minimal-message payload validation, sender checks, session sequencing/head tracking, reachability gating, store-backed bootstrap, `OBJECT` body verification, and a minimal peer-store sync driver now exist in `mycel-core`; capability-gated optional messages and broader peer interop remain |
| 8. Sync Workflow | Partial | `M4` | peer-store-driven first-time and incremental sync now prove shared verify/store flows through `mycel-core`, the CLI, and simulator positive-path coverage; optional flows and broader peer interop proof remain open |
...
| 11. CLI or API Surface | Partial | `M2` / `M3` / `M4` | verification, authoring, conservative merge authoring, editor-admission-aware reader inspection/render, governance inspect/list/publish, persisted governance index query surfaces, transcript-backed `sync pull`, and internal `sync peer-store` all exist; optional-message and broader replication behavior remain open |
| 12. Interop Test Minimum | Partial | `M1` / `M2` / `M4` | fixture isolation, reproducibility, stricter parser/replay smoke coverage, direct wire-envelope/signature/session tests, peer-store first-time / incremental sync proofs, and simulator positive-path coverage exist, but optional-message and broader peer-interop cases remain |
| 13. Ready-to-Build Gate | Partial | whole plan | replay, head selection, rebuild, conservative merge authoring, and peer-store sync proofs are green; parse closure and broader wire interop are not |
