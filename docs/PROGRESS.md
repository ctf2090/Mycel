...
Status: draft, refreshed after the recent wire-session reachability, store-backed bootstrap, and transcript-backed sync-pull batch
...
| `M4` | Early partial | wire envelope validation, `OBJECT` body verification, session reachability, store-backed bootstrap, and transcript-backed first-time / incremental sync-pull proofs | peer-driven end-to-end sync, capability-gated optional flows, and production replication behavior remain |
...
| 4. Signature Verification | Partial | `M1` / `M4` | object signature rules are in place, generic wire-envelope signature verification exists, and `OBJECT` body hash / `object_id` recomputation now runs in wire validation; remaining closure is broader shared-helper adoption plus peer-driven sync plumbing |
...
| 7. Wire Protocol | Partial | `M4` | canonical wire-envelope parsing, field validation, RFC 3339 checks, minimal-message payload validation, sender checks, session sequencing/head tracking, reachability gating, store-backed bootstrap, and `OBJECT` body verification now exist in `mycel-core`; capability-gated optional messages and broader peer interop remain |
| 8. Sync Workflow | Partial | `M4` | transcript-backed first-time and incremental `sync pull` flows now verify/store requested objects, and `WireSession` can bootstrap from known verified store state; minimal peer-driven sync and optional flows remain open |
...
| 11. CLI or API Surface | Partial | `M2` / `M3` / `M4` | verification, authoring, conservative merge authoring, editor-admission-aware reader inspection/render, governance inspect/list/publish, persisted governance index query surfaces, and transcript-backed `sync pull` all exist; peer-driven sync remains open |
| 12. Interop Test Minimum | Partial | `M1` / `M2` / `M4` | fixture isolation, reproducibility, stricter parser/replay smoke coverage, direct wire-envelope/signature/session tests, and transcript-backed first-time / incremental sync-pull smokes exist, but peer-driven sync and optional-message cases remain |
| 13. Ready-to-Build Gate | Partial | whole plan | replay, head selection, rebuild, conservative merge authoring, and transcript-backed sync-pull proofs are green; parse closure and peer-driven wire sync are not |
