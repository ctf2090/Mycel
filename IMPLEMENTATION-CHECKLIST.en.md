...
- [ ] Define and test the missing production replication behavior that still sits outside the current minimal sync proof. Scope: three specific sub-items below.
  - [ ] Re-sync idempotency: running sync twice when the reader is already current produces zero new stored objects, no errors, and stable accepted heads.
  - [ ] Depth-N incremental catchup: a reader at revision depth 1 catches up to a seed at depth ≥ 3 in a single HEADS/WANT pass, verifying that only the delta is fetched.
